    ...
    <div class="wrapper">
      ...
      <div class="panel-right">
        ...
        <form v-show="!isLogin" @submit.prevent="handleSubmit">
          ...
          <div ref="googleBtnRefRegister" class="google-btn-container"></div>
        </form>
      </div>
    </div>
...
<script setup>
  ...
  const googleBtnRefRegister = ref(null)
  ...
  const handleGoogleResponse = async (response) => {
    ...
    try {
      ...
      successMessage.value = `Bienvenido, ${data.user.name}!`
  ...
  const renderGoogleButton = (container) => {
  ...
  const initGoogle = () => {
    ...
    renderGoogleButton(googleBtnRef.value)
    renderGoogleButton(googleBtnRefRegister.value)
  }
  ...
  // Re-render button when switching tabs
  watch(isLogin, async () => {
    await nextTick()
    if (isLogin.value) {
      renderGoogleButton(googleBtnRef.value)
